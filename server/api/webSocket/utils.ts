...
export function switchplayers(
  ...
  const roundScores =
    currentPlayer.scores.legScores[currentLeg]?.roundScores[currentRound]
      ?.scores;
  ...
  if (roundScores && roundScores.length > 0) {
    ...
    if (
      roundTotal > 0 &&
      !currentPlayer.scores.trackedRounds?.[currentLeg]?.[currentRound]
    ) {
  ...
  nextPlayer.scores.legScores[match.currentLeg].roundScores[
    match.currentRound
  ] = {
    scores: [],
  };
...
export function updateScore(
  ...
  player.scores.legScores[match.currentLeg].roundScores[
    match.currentRound
  ].scores.push(points);
...
export function updateDartsCount(
  currentPlayer: Player,
  score: number,
  multiplier: number,
  match: Match
