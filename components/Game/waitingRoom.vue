  ...
  <div class="h-full w-full flex flex-col items-center justify-center overflow-auto">
    <div class="w-full max-w-6xl p-4 flex flex-col items-center">
      ...
      <div v-if="players.length < 2" class="text-center mb-4">
        <img src="/public/dartboard_small.png" alt="Waiting..." class="h-20 w-20 animate-spin-slow mb-2 mx-auto">
        <h2 class="text-3xl font-semibold text-white">Waiting for Opponent</h2>
        <p class="text-sm text-neutral-400">Please wait while we find an opponent</p>
      </div>
      <div v-if="players.length === 2" class="text-center mb-4">
        <h2 class="text-3xl font-semibold text-white mb-4">Match is ready to start</h2>
      ...
      <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="space-y-4">
          ...
          <div class="p-4 bg-neutral-900 rounded-lg shadow-md border-l-4 border-l-blue-500">
            <div class="flex justify-between items-center mb-2">
              <div class="text-lg font-medium">Game #{{ matchId.substring(0, 8) }}</div>
              <div class="font-medium px-2 py-0.5 rounded-full bg-blue-900/30 text-blue-300">
                Waiting
              </div>
            </div>
            ...
            <div class="flex flex-wrap w-full gap-2 mb-2">
              <div class="stat-badge">
                <Icon name="material-symbols:login-rounded" class="w-4 h-4 mr-1 text-blue-400" />
                <span>{{ settings.inMode }}</span>
              </div>
              <div class="stat-badge">
                <Icon name="material-symbols:logout-rounded" class="w-4 h-4 mr-1 text-green-400 scale-x-[-1]" />
                <span>{{ settings.outMode }}</span>
              </div>
              <div class="stat-badge">
                <Icon name="heroicons:chart-bar" class="w-4 h-4 mr-1 text-yellow-400" />
              ...
              <div class="stat-badge">
                <Icon name="material-symbols:check-rounded" class="w-4 h-4 mr-1 text-pink-400" />
            ...
            <div class="space-y-2">
              <div v-for="(player, index) in players" :key="index"
                ...
                <div class="flex items-center">
                  <div class="w-10 h-10 flex items-center justify-center bg-neutral-700 rounded-full mr-3">
              ...
              <div v-if="players.length < 2"
                ...
                <div class="flex items-center">
                  <div class="w-10 h-10 flex items-center justify-center bg-neutral-700/50 rounded-full mr-3">
                    <Icon name="mdi:account-plus" size="16" class="text-neutral-500" />
                  </div>
                  <p class="text-neutral-500 text-sm">Waiting for player...</p>
                </div>
                <div class="font-medium px-2 py-0.5 rounded-full bg-neutral-700/30 text-neutral-500">
                  Empty
                </div>
              </div>
            </div>
        ...
        <div class="p-4 bg-neutral-900 rounded-lg shadow-md">
          ...
          <div class="flex flex-col  space-y-4">
            <QRCode :value="shareableLink" :size="150" class="bg-white p-1 rounded" />
            ...
            <div class=" space-y-3">
              <div class="flex   bg-neutral-800 rounded-lg p-2">
                ...
                <button @click="copyLink"
                  ...
                  <Icon v-if="copied" name="material-symbols:check" class="text-green-400" size="14" />
                  <Icon v-else name="material-symbols:content-copy-outline" size="14" />
                </button>
              </div>

              <p class="text-neutral-400 text-sm">Send this link to invite a friend to your match</p>
            </div>
          </div>
        </div>
      </div>
