<template>
  <div class="flex h-screen">
    ...
    <div class="h-full w-full flex flex-col items-center">
      ...
      <div
        v-if="loadHistory && user"
        class="max-w-3xl w-full p-6 space-y-4 overflow-y-auto"
      >
        ...
        <div
          ...
          :class="{
            'border-l-4 border-l-green-500': userWonMatch(match),
            'border-l-4 border-l-red-500': !userWonMatch(match),
          }"
        ...
        >
          ...
          <div class="flex items-center justify-center space-x-3 mb-3">
            <div
              class="player-card flex-1"
              :class="{
                winner:
                  getCurrentUserOrFirstPlayer(match).scores.setsWon >
                  getOpponent(match).scores.setsWon,
              }"
            >
              <div class="flex flex-col">
                <span class="font-bold">{{
                  getCurrentUserOrFirstPlayer(match).username
                }}</span>
                <span class="text-neutral-400 text-xs"
                  >Average:
                  {{ getCurrentUserOrFirstPlayer(match).stats.average }} |
                  First9Avg:
                  {{ getCurrentUserOrFirstPlayer(match).stats.first9Average }} |
                  Checkout:
                  {{
                    getCurrentUserOrFirstPlayer(match).stats.checkoutPercentage
                  }}%</span
                >
              </div>
            ...
            >
              <div class="flex flex-col">
                ...
                <span class="text-neutral-400 text-xs"
                  >Average: {{ getOpponent(match).stats.average }} | First9Avg:
                  {{ getOpponent(match).stats.first9Average }} | Checkout:
          ...
          <!-- Match settings -->
          <div
            class="flex items-center justify-between text-sm text-neutral-500 mt-1 border-t border-neutral-800 pt-2"
          >
...
function getCurrentUserOrFirstPlayer(match: Match): Player {
  ...
  const currentUser = match.players.find(
...
async function getMatchHistory() {
  ...
  const { matches, error: apiError } = await $fetch<{
    matches: Match[];
    error: string;
  }>(`https://${window.location.host}/api/user/getMatchHistory`, {
    method: "POST",
    body: { userId },
  });
  if (apiError) {
