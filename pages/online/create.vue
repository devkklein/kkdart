      ...
      <form type="submit"
        ...
        <div class="mb-4 justify-center items-center w-full">
          <div class="w-full flex flex-col justify-start">
            <h1 class="text-white text-xl font-bold mb-2">Match Name:</h1>
        ...
        <div class="flex justify-center space-x-4 border-b-2 border-primary py-6">
          <button type="button" class="bg-secondary-300 border-2 rounded-lg p-4 w-1/4 hover:border-primary" :class="gameMode === 'x01' ? 'border-primary' : 'border-secondary-300'
            " @click="setMode('x01')">
            X01
          </button>
          <button type="button" class="bg-secondary-300 border-2 rounded-lg p-4 w-1/4 hover:border-primary" :class="gameMode === 'cricket' ? 'border-primary' : 'border-secondary-300'
            " @click="setMode('cricket')">
            Cricket
          </button>
          <button type="button" class="bg-secondary-300 border-2 rounded-lg p-4 w-1/4 hover:border-primary" :class="gameMode === 'clock' ? 'border-primary' : 'border-secondary-300'
            " @click="setMode('clock')">
            A round the Clock
          </button>
          <button type="button" class="bg-secondary-300 border-2 rounded-lg p-4 w-1/4 hover:border-primary" :class="gameMode === '121' ? 'border-primary' : 'border-secondary-300'
            " @click="setMode('121')">
            121
          </button>
        </div>
        ...
        <div class="flex-grow overflow-y-auto">
          <div class="bg-secondary-300 rounded-xl p-4">
            ...
            <ModeX01 :public="true" @sendMatchdata="updateSettings" v-if="gameMode === 'x01'" />
        ...
        <div class="flex w-full justify-between items-center mt-4">
          ...
          <button @click="createGame" type="button" class="bg-secondary-300 text-white rounded-lg p-2">
            Create Game
          </button>
        </div>
      </form>
...
const createGame = () => {
  ...
  ws.value = new WebSocket(
    `${protocol}${window.location.host}/api/webSocket/ws`
  );
  ws.value.onopen = () => {
